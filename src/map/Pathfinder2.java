...
public class Pathfinder2 {
	...
	public Region getNearestOwnedRegionToSuperRegion(SuperRegion superRegion,
			String playerName) {
		...
		for (Iterator<Region> iterator = new BFSIterator(
				superRegion.getSubRegions()); iterator.hasNext();) {
	...
	public Path getShortestPathToSuperRegionFromRegionOwnedByPlayer(
			SuperRegion superRegion, String playerName) {
		Region origin = null;
		Region target = null;

		for (Iterator<Region> iterator = new BFSIterator(
				map.getOwnedRegions(playerName)); iterator.hasNext();) {
			Region next = iterator.next();
	...
	public Region getNearestOwnedRegion(Region origin, String playerName) {
		for (Iterator<Region> iterator = new BFSIterator(origin); iterator
				.hasNext();) {
			Region next = iterator.next();
	...
	public List<Region> getPlayerInnerRegions(Map map, String playerName) {
		...
		outer: for (Iterator<Region> iterator = new BFSIterator(
				map.getOwnedRegions(playerName)); iterator.hasNext();) {
			Region next = iterator.next();

			for (Region neighbor : next.getNeighbors()) {
				System.out.println(next.getId() + " neighbor "
